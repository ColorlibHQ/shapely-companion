/**
 * Verify every gettext call uses the plugin's text domain.
 *
 * Replaces grunt-checktextdomain. Catches the two mistakes that actually ship:
 * a wrong or typo'd domain, and a missing domain argument, which silently drops
 * the string from translation.
 */
import { readdir, readFile } from 'node:fs/promises';
import path from 'node:path';
import { ROOT, TEXT_DOMAIN } from './plugin-meta.mjs';

const ALLOWED = new Set([TEXT_DOMAIN]);
...
for (const [abs, rel] of files) {
  ...
  while ((m = re.exec(src)) !== null) {
    ...
    if (!literal) continue; // variable/constant domain -- not statically checkable
    const domain = literal[1] ?? literal[2];
    if (!ALLOWED.has(domain)) {
      problems.push(`${rel}:${line}  ${fn}() uses domain '${domain}' (expected '${TEXT_DOMAIN}')`);
    }
  }
}
...
console.log('  ok - all text domains valid');
